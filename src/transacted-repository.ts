...
import { QueryRunnerException } from "./queryrunner.exception";

export class TransactedRepository<Entity extends ObjectLiteral> {
  // -------------------------------------------------------------------------
  // Public Properties
  // -------------------------------------------------------------------------
  ...
  get queryRunner(): QueryRunner | undefined {
    if (this.request.queryRunners && this.request.queryRunners.length > 0) {
      return this.request.queryRunners[this.request.queryRunners.length - 1];
    }
    throw new QueryRunnerException("QueryRunner is required");
  }

  get manager(): EntityManager {
    return this.queryRunner?.manager as EntityManager;
  }
  ...
  constructor(target: EntityTarget<Entity>, request: any) {
    this.target = target;
